...
export function FloatingChatWidget() {
  ...
  useEffect(() => {
    ...
    if (savedOpenState === "1") setIsOpen(true);
  }, []);
  ...
  function handleContextMenu(event: React.MouseEvent, message: ChatMessage, mine: boolean) {
    if (!mine) return;
        ...
        <section className="panel flex h-[76vh] max-h-[640px] w-[370px] max-w-[calc(100vw-1.5rem)] flex-col overflow-hidden rounded-[1.8rem] border border-white/70 bg-[#fbfaf9]/95 shadow-[0_28px_60px_rgba(22,28,45,0.28)] backdrop-blur-xl">
          ...
          {!isSignedIn ? (
            ...
            <>
              {contextMenu ? (
                <div
                  className="fixed z-[80] flex items-center gap-1 rounded-lg border border-[#ece9e8] bg-white p-1 shadow-panel"
                  ...
                  {messages.map((message) => {
                    ...
                    return (
                      <div
                        ...
                        key={message.id}
                        onContextMenu={(event) => handleContextMenu(event, message, mine)}
                      >
